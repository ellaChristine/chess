...
public class ChessPiece {
    ...
    public Collection<ChessMove> pieceMoves(ChessBoard board, ChessPosition myPosition) {
        ...
        //make separate methods for all the chess pieces
        ...
        if (piece.getPieceType() == PieceType.PAWN) {
            if (piece.pieceColor == ChessGame.TeamColor.WHITE) {
                ...
                return pawnMoves(board,myPosition,move1,move2,2,7, capture1,capture2);
            }
    ...
    private Collection<ChessMove> pawnMoves(ChessBoard board, ChessPosition myPosition, ChessPosition move1,
                                            ChessPosition move2,Integer startRow,Integer promotionRow,
                                            ChessPosition capture1, ChessPosition capture2){
        ChessPiece piece = board.getPiece(myPosition);
        List<ChessMove> moves = new ArrayList<>();
        if (myPosition.getRow() == startRow) {
            ChessPiece destination1 = board.getPiece(move1);
            ChessPiece destination2 = board.getPiece(move2);
            ...
            ChessPiece promotionDestination = board.getPiece(move1);
            if (promotionDestination == null) {
                moves.add(new ChessMove(myPosition, move1, PieceType.QUEEN));
                moves.add(new ChessMove(myPosition, move1, PieceType.BISHOP));
                moves.add(new ChessMove(myPosition, move1, PieceType.ROOK));
                moves.add(new ChessMove(myPosition, move1, PieceType.KNIGHT));
            }

        } else {
            ChessPiece destination = board.getPiece(move1);
            if (destination == null) {
                moves.add(new ChessMove(myPosition, move1, null));
            }
        }
